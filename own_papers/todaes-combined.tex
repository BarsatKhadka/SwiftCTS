...
Features are scale-invariant: logarithmic transforms handle dynamic range,
dimensionless ratios capture universal density patterns, and
knob$\times$circuit interaction terms ground the model in circuit physics
...
Three independent gradient-boosted heads predict power (XGBoost, 20 features),
wirelength (blended LightGBM/Ridge, 75 features), and skew (LightGBM,
15 geometric-knob features). Independence avoids the split-selection bias that
a shared trunk would introduce when objectives have different variance scales.
Each target is expressed as a scale-free log-ratio so the model captures
within-design knob sensitivity rather than cross-design absolute magnitude.

\paragraph{K-shot calibration.}
\label{sec:kshot}
Zero-shot predictions on unseen designs carry a constant scalar offset from
the new design's absolute size and activity level.
Given $K$ actual CTS runs on the target placement, a geometric-mean scaling
factor corrects this offset without arithmetic bias:
\begin{equation}
  k_\text{cal} = \exp\!\left(\frac{1}{K}\sum_{i=1}^{K}
    \log\!\left(\frac{y_\text{true}^{(i)}}{\hat{y}_\text{pred}^{(i)}}\right)\right)
\end{equation}
This scalar is broadcast as $\hat{y}_\text{calibrated} = k_\text{cal}\times\hat{y}_\text{pred}$.
A single reference run ($K{=}1$) suffices in practice.

\paragraph{Search.}
The four CTS knobs are the only dynamic variables per placement.
We tile a base layout feature vector $N$ times, patch knob columns via
vectorized operations, normalize the resulting predicted cost matrix
$\mathbf{C}\in\mathbb{R}^{N\times 3}$, and run NSGA-II
(100K evaluations, average 8.9\,s wall clock).

\subsection{Evaluation Protocol}

Experiments use 5,520 CTS-Bench evaluations across six designs (training:
AES, PicoRV32, SHA-256, EthMAC; OOD: JPEG encoder at $5\times$ larger,
ZipDiv at $21\times$ smaller).
We report MAPE for power and wirelength, and MAE in nanoseconds for skew.
\textit{Leave-One-Design-Out (LODO)} tests in-distribution generalization;
\textit{Out-of-Distribution (OOD)} stress-tests absolute structural limits.
...
The initial CTS-Bench release targets Sky130 (130\,nm). We are actively
extending the dataset to ASAP7 (7\,nm predictive) and NanGate45 (45\,nm)
to enable cross-PDK generalization studies; the multi-PDK generation pipeline
is released alongside this paper. Coverage of commercial nodes requires
anonymized structures and remains future work.
...
We presented CTS-Bench, an open, large-scale benchmark dataset and automated
HPC pipeline for AI-driven Clock Tree Synthesis research.
CTS-Bench provides 10,000+ labeled CTS evaluations across 19 RTL designs,
covering multiple technology nodes (Sky130, ASAP7, NanGate45), four CTS
knob dimensions, and 15 ground-truth QoR metrics per run.
An optional graph extraction module converts each placement+SAIF pair into
PyTorch Geometric tensors, supporting GNN-based research alongside tabular
surrogate models.

The SwiftCTS surrogate demonstrates that a dataset of this scale enables
accurate, sub-millisecond CTS quality prediction and 100K-evaluation design
space exploration in under 10 seconds---a $67\times$ reduction in search cost
over conventional approaches.
We release all data, code, and the full HPC generation pipeline to lower the
barrier for the community to build on, extend, and benchmark against
CTS-Bench.
