...
CTS_BENCH_ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
...
""".format(
    ...
    result = subprocess.run([
        ...
        "-v", f"/Users/barsat/.volare:/Users/barsat/.volare",
...
def run_single_experiment(design_name, clock_period, clock_port, top_module=None, max_core_util=70):
    ...
    pl_density = round((core_util / 100.0) + 0.05 + random.uniform(0.0, 0.20), 2)
    pl_density = min(pl_density, 0.99)
    ...
    if design_name in DESIGNS_WITH_INCLUDES:
        config["VERILOG_INCLUDE_DIRS"] = [
            os.path.join(CTS_BENCH_ROOT, "designs", design_name, "rtl")
        ]

    # Write config to a temp file in CTS-Bench root (must be inside mounted volume)
    config_path = os.path.join(CTS_BENCH_ROOT, "tmp_ol_config.json")
    with open(config_path, "w") as f:
        json.dump(config, f, indent=4)

    print(f"Running OpenLane (dockerized) for {design_name} tag={tag}")
    result = subprocess.run(
        [
            "python3", "-m", "openlane",
            "--docker-mount", "/Users/barsat/.volare",
            "--docker-no-tty",
            "--dockerized",
            "-T", "OpenROAD.DetailedPlacement",
            # Skip lint and synth-check steps — IWLS RTL has benign warnings
            "-S", "Verilator.Lint",
            "-S", "Checker.LintTimingConstructs",
            "-S", "Checker.LintErrors",
            "-S", "Checker.LintWarnings",
            "-S", "Checker.YosysSynthChecks",
            "--pdk-root", MY_PDK_ROOT,
            "--run-tag", tag,
            config_path,
        ],
        cwd=CTS_BENCH_ROOT,
    ...
    try:
        ...
        pass  # OpenLane may have moved the config during Docker processing

    if result.returncode != 0:
        print(f"OpenLane run failed for {tag} (exit {result.returncode})")
        return None
    ...
    latest_run_file = os.path.join(CTS_BENCH_ROOT, "latest_run.txt")
